...
install -m 755 swiftlint/swiftlint swiftformat/swiftformat "$DEST/bin/"

if [[ -n "${GITHUB_PATH:-}" ]]; then
    echo "$DEST/bin" >> "$GITHUB_PATH"
    echo "$DEST/xcodegen/bin" >> "$GITHUB_PATH"
fi
echo "Installed xcodegen $XCODEGEN_VERSION, swiftlint $SWIFTLINT_VERSION, swiftformat $SWIFTFORMAT_VERSION to $DEST"
